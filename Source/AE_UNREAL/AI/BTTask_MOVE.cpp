...
void UBTTask_MOVE::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DelataSeconds)
{
	...
	{
		FVector PawnWorldPos = GetGlobalCharacter(OwnerComp)->WorldPosition;
		FVector PawnPos = GetGlobalCharacter(OwnerComp)->GetActorLocation();
		FVector TargetPos = TargetActor->GetActorLocation();

		FVector DirWorld = TargetPos - PawnWorldPos;
		FVector DirLocal = TargetPos - PawnPos;

		GetGlobalCharacter(OwnerComp)->AddMovementInput(DirLocal);
		...
		if (SearchRange*1.5f < DirWorld.Size())
		{
			// 원재 라리로 돌아가고
			SetStateChange(OwnerComp, AIState::RETURN);
			return;
